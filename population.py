...
class Population:
    ...
    def update_live_players(self):
        for p in self.players:
            if p.alive:
                #Calculate the number of pipes passed
                p.score()
                #print(int(p.pipesPassed))
                p.look()
                p.think()
    ...
    def next_gen(self):
        if self.extinct():
            ...
            best_player.save_genome('SavedGameInfo.txt')
    ...
    def collect_weights_statistics(self):
        all_weights = []
        ...
        variance = sum((i - mean_weight) ** 2 for i in all_weights) / len(all_weights)

        return mean_weight, variance
